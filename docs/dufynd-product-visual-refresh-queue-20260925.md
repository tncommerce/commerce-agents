...
| --- | --- | --- |
| Creed Absolu Aventus 100 ml | The live DUFYND scene still reads **75 ML / 2.5 FL.OZ.** and remains invalid as product truth. A new black, volume-neutral candidate avoids the contradictory size marking, but Creed's current 100 ml page still uses variant-ambiguous imagery. | Candidate stored at `/products/candidates/creed-absolu-aventus-p0-v1.webp`; keep it inactive until the exact 100 ml variant is resolved or a volume-neutral exact-reference gate can be completed. |
