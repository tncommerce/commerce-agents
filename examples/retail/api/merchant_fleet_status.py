...
def build_fleet_status_summary(
    ...
    *,
    ...
    summaries.sort(key=lambda summary: summary.job_id.casefold())

    health_items = [evaluate_job_health(summary) for summary in summaries]

    health_ok = sum(item.severity == "ok" for item in health_items)
    health_info = sum(item.severity == "info" for item in health_items)
    health_warning = sum(item.severity == "warning" for item in health_items)
    health_blocked = sum(item.severity == "blocked" for item in health_items)
    ...
    for summary in summaries:
        item = build_job_attention_item(summary)

        if item is not None:
            attention_items.append(item)

    attention_job_ids = [item.job_id for item in attention_items]

    return MerchantFleetStatusSummary(
        total_jobs=len(summaries),
        ready=sum(summary.job_state == "ready" for summary in summaries),
        disabled=sum(summary.job_state == "disabled" for summary in summaries),
        not_ready=sum(summary.job_state == "not_ready" for summary in summaries),
        approval_required=sum(summary.job_state == "approval_required" for summary in summaries),
        latest_run_review=sum(summary.latest_run_status == "review" for summary in summaries),
