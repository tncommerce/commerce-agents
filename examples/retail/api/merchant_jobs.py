...
def load_merchant_jobs(
    ...
    jobs = [MerchantJobProfile.model_validate(row) for row in rows]

    normalized_ids = [job.job_id.strip().casefold() for job in jobs]
...
def get_merchant_job(
    ...
    raise KeyError(f"Unknown merchant job: {job_id}")
    ...
    *,
    ...
    if not job.config.dry_run:
        approval_file = (
            ...
            else Path("examples/retail/data/merchant_job_approvals.json")
        )

        approvals = load_job_approvals(approval_file)
        ...
        if not approved:
            return MerchantOperationalRun(
                ...
                exit_code=20,
                import_exit_code=20,
                reasons=["dry_run_approval_required"],
            )
