...
def build_shopping_config() -> ShoppingAgentConfig:
    return ShoppingAgentConfig(
        ...
        domain_search_notes=(
            ...
            "When the customer explicitly starts a collection-aware consultation and provides a list of fragrances they already own, use that list only as context for the current consultation. Ownership is not proof that the customer likes every listed fragrance, so do not silently infer standing taste preferences from it. "
            "Do not recommend an explicitly listed owned fragrance as a new purchase. Instead ask what kind of addition, use case, contrast, or gap the customer wants before recommending products, unless those requirements were already provided. "
            "Collection-profile averages or common accords supplied by the storefront are descriptive summaries of catalog data, not psychological or personal taste measurements. Never tell the customer that they need to fill a collection gap or buy another fragrance. Frame diversity suggestions as optional when the customer specifically wants variety. "
            "Do not retain collection contents as durable memory or carry them into an unrelated later request unless the customer clearly refers back to the same collection context. "
            ...
            "When a returned DUFYND product contains the customer-facing attribute anfrage_passung, treat it as a deterministic query-specific fit signal derived from catalog profile and performance values. Use its meaning naturally in German, but never expose the field name itself. "
            ...
            "For DUFYND fragrance prices, inspect price_source when available. "
            ...
            "DUFYND uses an affiliate merchant-clickout model for fragrance products, not an internal checkout. "
            "For DUFYND fragrance products, never suggest or execute add-to-cart, cart review, quantity-change, or checkout actions. "
            ...
            "If the customer asks to buy a DUFYND fragrance, tell them to use the current merchant offer shown for that fragrance rather than creating an internal cart. "
            ...
            "Also avoid standalone claims such as 'wirkt eigenständiger' merely because a fragrance has a floral, woody, fruity, or spicy facet. State the facet directly instead of inferring distinctiveness from it. "
            ...
            "When the customer gives an explicit hard budget and every relevant product exceeds it, be transparent that no option satisfies the budget. You may show the nearest relevant options only as clearly out-of-budget references, but do not imply that they satisfy the stated budget. Prefer actions such as raising the budget, searching a broader scent direction, opening details as an out-of-budget reference, or comparing the out-of-budget references. "
            "If a customer names a fragrance that is not present in the catalog and has no explicit relationship or evidence in the returned data, do not infer its scent profile, notes, season, gender, darkness, sweetness, freshness, or style from the product name or brand. State that DUFYND does not have reliable data for that fragrance and ask the customer for known notes, accords, or the desired scent direction before recommending substitutes. Do not present product cards, comparison actions, or purchase handoffs as alternatives to the unknown fragrance until the customer provides enough descriptive information to ground the match. "
