...
BRAND_ROOT = Path("examples/retail/storefront-web/public/brand")
BRAND_PACK = Path("examples/retail/data/dufynd_brand_pack.json")
...
def test_dufynd_brand_master_assets_exist_and_are_current() -> None:
    ...
    assert expected == {path.name for path in BRAND_ROOT.glob("*.svg")}

    for filename in expected:
        text = (BRAND_ROOT / filename).read_text(encoding="utf-8")
        assert "DUFYND" in text
        assert "SCENTAI" not in text


def test_dufynd_brand_pack_references_existing_assets() -> None:
    payload = json.loads(BRAND_PACK.read_text(encoding="utf-8"))
    ...
    assert any("Never ask a video model" in rule for rule in payload["ai_video_rules"])
